...
export default function KPIDetailModal({ type, onClose, layoutId }: PropsWithLayout) {
  ...
  const [studentForm, setStudentForm] = useState({ categoryId: "", studentRole: "aluno" as "aluno" | "observador" | "professor", notes: "" });
              ...
              {paid.slice(0, 6).map(p => {
                ...
                return (
                  <div key={p.id} className="flex items-center justify-between p-2.5 rounded-lg bg-black/30 border border-zinc-900/50">
                    ...
                    <span className="text-sm font-bold text-white">{formatBRL(p.amount)}</span>
                  </div>
                );
              })}
            </div>
                  ...
                  return (
                    <div className="bg-black/40 border border-zinc-800 rounded-xl p-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          <label className="text-[10px] uppercase font-bold text-zinc-500 mb-2 block">Papel do Aluno</label>
                          <div className="space-y-2">
                            {[
                              { value: "aluno" as const, label: "🏐 Aluno", desc: "Treina, ganha XP, acesso completo" },
                              { value: "observador" as const, label: "👀 Observador", desc: "Apenas feed e perfil (sem XP)" },
                              { value: "professor" as const, label: "👨‍🏫 Professor", desc: "Acesso ao cockpit admin" },
                            ].map(({ value, label, desc }) => (
                              <label key={value} className="flex items-start gap-2 p-2.5 rounded-lg bg-zinc-900/30 border border-zinc-800 cursor-pointer hover:border-[#EAB308]/30 transition-colors">
                                <input
                                  type="radio"
                                  name="studentRole"
                                  value={value}
                                  checked={studentForm.studentRole === value}
                                  onChange={(e) => setStudentForm(p => ({ ...p, studentRole: e.target.value as "aluno" | "observador" | "professor" }))}
                                  className="mt-0.5"
                                />
                        ...
                        <div className="flex gap-2 pt-2">
                          ...
                          <button onClick={() => {
                            ...
                            updateStudent(s.id, { categories: [studentForm.categoryId], studentRole: studentForm.studentRole, notes: studentForm.notes });
                            approveStudent(s.id);
                            toast(`✅ ${s.name.split(" ")[0]} foi matriculado como ${studentForm.studentRole === "aluno" ? "aluno" : studentForm.studentRole === "observador" ? "observador" : "professor"}!`);
                      ...
                      <div key={s.id} className="flex items-center justify-between p-2.5 rounded-lg bg-black/30 border border-zinc-900/50">
                        ...
                        <div className="flex gap-2">
                          ...
                          <motion.button whileTap={{ scale: 0.9 }}
                            onClick={() => { setApprovingStudent(s.id); setStudentForm({ categoryId: "", studentRole: "aluno", notes: "" }); }}
