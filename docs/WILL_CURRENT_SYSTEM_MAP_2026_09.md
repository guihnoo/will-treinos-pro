...
| `/will/court` | Painel operacional da aula (quadra) | **admin/owner apenas** — ver nota abaixo |
| `/will/court/[lessonId]/live` | Coaching ao vivo (`LiveLessonCoachPanel`) | **admin/owner apenas** — ver nota abaixo |
| `/will/evaluations/templates` | Editor de templates de avaliação | admin |
| `/will/status` | Healthcheck (crons, DB, push, bot) | admin |

> **Correção desta revisão — acesso real a `/will/*`:** a versão anterior deste documento listava `/will/court` e `/will/court/[lessonId]/live` como acessíveis também a professor. **Isso estava incorreto no estado atual do código.** Confirmado em `middleware.ts` (prefixo `/will/:path*` guardado) + `src/domain/v1/rbac.ts`: `PREFIX_ROLE_GUARD["/will"] = ["will_owner"]` — apenas o role `will_owner` (admin/dono) passa em `canAccessPrefix(role, "/will")`; qualquer outro role (incluindo `professor`) é redirecionado para `/dashboard?denied=/will`. O role de professor está mapeado ao prefixo `/prof` (`PREFIX_ROLE_GUARD["/prof"] = ["professor"]`), não a `/will`. **No estado atual, `/will/court` e `/will/court/[lessonId]/live` são acessíveis somente a admin/owner** — mesmo que o conteúdo dessas telas (painel de quadra, coaching ao vivo) seja conceitualmente pensado para o dia a dia de um professor. Essa distinção entre "para quem a tela foi pensada" e "quem o RBAC de fato deixa entrar" é tratada como mudança futura de produto no `docs/WILL_PRODUCT_SYSTEM_ARCHITECTURE_2_0.md` (Seção 6, Sprint 7) — não uma correção deste PR.
...
| **Professor/Coach** (`professor`) | `/dashboard` → `CoachHome` | Prancheta da quadra, avaliações, check-in (dentro de `/prof/*` e telas embutidas no `CoachHome`) | **Sem** `/financeiro`, **sem** `/will/*` (middleware — `PREFIX_ROLE_GUARD["/will"] = ["will_owner"]`, professor mapeado só a `/prof`) | Alunos da própria turma, avaliações, presença |
...
- O slot da noite é coberto por **workaround via GitHub Actions**: `.github/workflows/cron-evening.yml` dispara `GET /api/cron/orchestrator-evening` às 21h UTC (18h BRT), autenticado com `Authorization: Bearer ${{ secrets.CRON_SECRET }}`. **Correção desta revisão:** a versão anterior deste documento dizia `POST` — incorreto. O `curl` no workflow não usa `-X` nem envia corpo, então o método real é `GET` (o padrão do `curl`), e `src/app/api/cron/orchestrator-evening/route.ts` exporta somente `GET` (confirmado lendo o arquivo — não há `export async function POST`).
- **Confirmado pelo código:** as outras 9 rotas de cron são de fato chamadas internamente pelos dois orquestradores. `orchestrator-morning` despacha (com `fetch` + `Authorization: Bearer ${CRON_SECRET}`) para `birthday-reminder`, `daily-reminder`, `onboarding-reminder` (diários), `payment-reminder` (dias 5 e 20) e `monthly-report` (dia 1). `orchestrator-evening` despacha para `absence-reminder`, `fomo-reminder`, `post-lesson-feedback` (diários) e `weekly-report` (só sexta-feira, `dow === 5`).
- **Confirmado pelo código:** todas as 11 rotas de `cron/*` (incluindo os dois orquestradores) validam `req.headers.get("authorization") === \`Bearer ${CRON_SECRET}\`` (ou `Authorization`, variando a capitalização entre arquivos) no início do handler, retornando `401` se o header não bater — cada rota faz a checagem inline (não há um helper compartilhado, mas a proteção existe e está presente em 100% das rotas inspecionadas). **Não existe rota de `cron/*` sem essa checagem.**
- **Edge case real documentado nesta revisão (ver também AS-IS Seção 16, novo item P1):** os dois orquestradores declaram `const CRON_SECRET = process.env.CRON_SECRET ?? ""`. Se a env var `CRON_SECRET` estiver ausente/mal configurada no ambiente de produção, `CRON_SECRET` vira string vazia e a checagem passa a comparar contra `` `Bearer ` `` (token vazio) — ou seja, um request com `Authorization: Bearer ` (valor vazio) passaria pela checagem nesse cenário específico. **No estado normal, com `CRON_SECRET` configurado (é o caso hoje — o workflow do GitHub falha explicitamente com `exit 1` se o secret estiver ausente do lado do GitHub Actions), a rota está protegida.** O risco existe somente se a env var estiver ausente/mal configurada no runtime da Vercel — cenário não confirmado como ocorrendo hoje, mas que a rota em si não impede explicitamente (não há um `if (!process.env.CRON_SECRET) return 500` fail-closed nas rotas).
...
- **Aula → avaliação → XP:** aula criada em `/agenda` → check-in via QR (`/checkin/[lessonId]`) → aula rodada em `/will/court` ou coaching ao vivo (`/will/court/[lessonId]/live`) — **hoje restrito a admin/owner pelo RBAC (ver nota da Seção 3)**, ainda que a tela sirva conceitualmente ao dia a dia de professor — → avaliação lançada (prancheta) → XP calculado e registrado em `xp_log` → aparece no Feed/Ranking/Cards do aluno.
...
4. **`CRON_SECRET` ausente faria as rotas `cron/*` falharem aberto, não fechado — configuração de segurança / fail-closed** — **investigado nesta correção**, lendo `orchestrator-morning/route.ts` e `orchestrator-evening/route.ts`. Ambos declaram `const CRON_SECRET = process.env.CRON_SECRET ?? ""` e validam `Authorization === \`Bearer ${CRON_SECRET}\``. **No estado normal, com a env var configurada (é o caso hoje em produção), a rota está protegida.** O risco é um edge case: se `CRON_SECRET` estivesse ausente/mal configurada no runtime da Vercel, `CRON_SECRET` viraria `""` e um request com `Authorization: Bearer ` (token vazio) passaria pela checagem — nenhuma das rotas tem um `if (!process.env.CRON_SECRET) return 500` explícito antes de comparar. O workflow do GitHub Actions (`cron-evening.yml`) já tem essa proteção do próprio lado dele (`if [ -z "$CRON_SECRET" ]; then exit 1`), mas isso não protege a rota contra outras formas de chamada. Hardening futuro (Sprint 2 — Segurança Funcional): fazer a rota falhar explicitamente (500, não seguir para a comparação) se `process.env.CRON_SECRET` não existir.

### P2 — Dívida técnica importante

6. **`WillCockpit.tsx` como God Component (3687 linhas)** — **reclassificado nesta correção** (estava listado como P0). Justificativa: concentra a maior parte da lógica/UI do painel admin, o que aumenta o risco de regressão cruzada e dificulta revisão/teste isolado — mas isso é um risco de **manutenibilidade e velocidade de desenvolvimento futuro**, não uma vulnerabilidade ativa, vazamento de dado ou indisponibilidade hoje. Não há evidência de que o tamanho do arquivo, por si só, cause um incidente em produção.
7. **Ausência de helper Supabase server-side compartilhado** — **reclassificado nesta correção** (estava listado como P0). 44 rotas de API recriam inline o padrão "client anon para validar JWT + client service role para consultar", com pequenas variações entre elas. Isso é dívida arquitetural real — qualquer correção de segurança nesse padrão (como a feita no Sprint 0A) precisa ser replicada rota a rota, aumentando o risco de uma futura inconsistência — mas hoje não há evidência de que exista de fato uma rota inconsistente/vulnerável por causa dessa duplicação (o item 1 desta lista, investigado a fundo, mostrou que o padrão observado em `leaderboard` é intencional, não um esquecimento).
8. **~40 arquivos `.md` legados na raiz do repositório** (auditorias, resumos de fase, prompts de outras ferramentas — `ANALISE_COMPLETA_PROJETO.md`, `PHASE7_VALIDATION_REPORT.md`, `STITCH_*`, etc.) sem curadoria — dificulta saber qual documentação é a fonte de verdade atual (`CLAUDE.md` e `WILLPRO_MASTER_MEMORY.md` são, mas isso não é óbvio para quem chega agora).
9. **`fix.js` na raiz** — script de patch avulso com caminho absoluto hardcoded para `C:\Users\monte\Desktop\will-treinos-pro\...` (repositório-irmão, fora deste worktree) — código morto, mas mostra o padrão de "scripts de debug soltos no repo" que já gerou o incidente de segurança do Sprint 0D-C.
10. **Duplicação de nome/responsabilidade:** `LiveLessonCoachPanel.tsx` existe em dois locais (`src/components/` e `src/components/will/`, 443 e 378 linhas); `csvExport.ts` e `exportCsv.ts` como libs paralelas; subsistema de XP fragmentado em 4 arquivos lib (`xpAntiCheat`, `xpEventLogger`, `xpIntegration`, `xpLogger`) com sobreposição de nome — todos candidatos a consolidação, não confirmado o grau real de sobreposição sem leitura de conteúdo linha a linha.
11. **`CLAUDE.md` desatualizado em relação ao código real** — lista de contexts incompleta (faltam `TrainingProvider` e `GamificationProvider`); é o tipo de deriva que se acumula quando a documentação não é atualizada no mesmo commit que o código.
...
- Leitura de conteúdo linha a linha para confirmar duplicações suspeitas (Seção 16, itens 10 e 12) — apenas apontadas por nome/tamanho.
